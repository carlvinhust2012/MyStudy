...
```mermaid
graph TB
    subgraph H100_GPU["H100 SXM5 GPU (132 SMs, 16,896 CUDA Cores)"]
        direction TB

        subgraph GPC_Layer["GPC 层 — 图形处理集群"]
            GPC0["GPC 0"]
            GPC1["GPC 1"]
            GPC2["GPC 2"]
            GPC3["GPC 3"]
        ...
        subgraph MC_Layer["内存控制器层"]
            MC0["MC 0 2x HBM3 Stacks"]
            MC1["MC 1 2x HBM3 Stacks"]
            MC2["MC 2 2x HBM3 Stacks"]
            MC3["MC 3 2x HBM3 Stacks"]
            MC4["MC 4 2x HBM3 Stacks"]
            MC5["MC 5 2x HBM3 Stacks"]
        end

        L2["L2 Cache 50 MB 128B Cache Line"]

        TMA["TMA Tensor Memory Accelerator"]

        NVLink_IF["NVLink 4 18 Links 900 GB/s"]

        PCIe_IF["PCIe Gen5 x16 64 GB/s"]

        GPC_Layer <-->|"128B line"| L2
        L2 <-->|"6144-bit bus"| MC_Layer
        GPC_Layer -.->|"异步拷贝"| TMA
        TMA <--> L2
        L2 <--> NVLink_IF
        L2 <--> PCIe_IF
    ...
    subgraph HBM_Stacks["HBM3 Stacks (80 GB)"]
        HBM0["Stack 0 16 GB"]
        HBM1["Stack 1 16 GB"]
        HBM2["Stack 2 16 GB"]
        HBM3["Stack 3 16 GB"]
    ...
    style H100_GPU fill:#1a1a2e,stroke:#e94560,color:#fff
    style L2 fill:#0f3460,stroke:#e94560,color:#fff
    style TMA fill:#16213e,stroke:#e94560,color:#fff
```
...
graph TB
    ...
    subgraph TPC_Detail["TPC 内部 (示例 TPC 0)"]
        SM0["SM 0"]
        SM1["SM 1"]
        MMU["MMU 页表管理"]
    end
    ...
    style GPC fill:#533483,stroke:#e94560,color:#fff
    style TPC_Detail fill:#0f3460,stroke:#e94560,color:#fff
```
...
```mermaid
graph TB
    subgraph SM["Single SM — Streaming Multiprocessor"]
        direction TB

        subgraph Sched["调度层 — Warp Scheduler"]
            WS0["Warp Scheduler 0 管理 16 Warps"]
            WS1["Warp Scheduler 1 管理 16 Warps"]
            WS2["Warp Scheduler 2 管理 16 Warps"]
            WS3["Warp Scheduler 3 管理 16 Warps"]
        end

        subgraph Dispatch["Dispatch 层"]
            D0["Dispatch Unit 0 2 instr/clock"]
            D1["Dispatch Unit 1 2 instr/clock"]
            D2["Dispatch Unit 2 2 instr/clock"]
            D3["Dispatch Unit 3 2 instr/clock"]
        end

        subgraph Exec["执行单元层"]
            direction LR
            subgraph FP32Pipe["FP32 Pipeline"]
                FP32ALU["128x FP32 ALU 128 ops/clock"]
            end
            subgraph FP64Pipe["FP64 Pipeline"]
                FP64ALU["64x FP64 ALU 64 ops/clock"]
            end
            subgraph INT32Pipe["INT32 Pipeline"]
                INT32ALU["64x INT32 ALU 64 ops/clock"]
            end
            subgraph SFUPipe["SFU Pipeline"]
                SFU["32x SFU sin/cos/exp/log"]
            end
            subgraph TensorPipe["Tensor Core Pipeline"]
                TC["4x Tensor Core FP8: 1024 ops/clock FP16: 512 ops/clock TF32: 256 ops/clock"]
            end
            subgraph LSDSPipe["Load/Store Pipeline"]
                LSU["32x LD/ST Units 256 B/clock (shared) 128 B/clock (global)"]
            end
        end

        subgraph RF["寄存器文件"]
            RegFile["Register File 256 KB (65,536 x 32bit)"]
        end

        subgraph SMem["片上存储"]
            L1Cache["L1 Cache 可配置: 0~164 KB"]
            SharedMem["Shared Memory 可配置: 92~256 KB"]
        end

        IB["Instruction Buffer 64 KB"]
    ...
    style SM fill:#16213e,stroke:#e94560,color:#fff,stroke-width:2px
    style Sched fill:#533483,color:#fff
    style Dispatch fill:#533483,color:#fff
    style Exec fill:#1a1a2e,color:#fff
    style RF fill:#0f3460,color:#fff
    style SMem fill:#0f3460,color:#fff
```
...
```mermaid
graph LR
    subgraph Pipeline["SM 指令流水线 (~10-14 级)"]
        IF["1. Instruction Fetch 指令获取 从 Instruction Buffer"]
        ID["2. Instruction Decode 指令解码 解析操作码与操作数"]
        RR["3. Register Read 寄存器读取 Scoreboard 检查依赖"]
        IS["4. Issue 发射 Warp Scheduler 选择就绪 Warp"]
        EX["5. Execute 执行 路由到对应功能单元"]
        WB["6. Writeback 写回 结果写入寄存器文件"]
    end

    IF --> ID --> RR --> IS --> EX --> WB

    style Pipeline fill:#16213e,stroke:#e94560,color:#fff
    style IF fill:#533483,color:#fff
    style ID fill:#533483,color:#fff
    style RR fill:#0f3460,color:#fff
    style IS fill:#e94560,color:#fff
    style EX fill:#e94560,color:#fff
    style WB fill:#0f3460,color:#fff
```
